...
def istft_thread(i, result_dir, preds, X, fname):
  config = Config()
...
def plot_result(epoch, num, fig_dir, preds, X, Y, mode="valid"):
  ...
  for i in range(min(num, len(X))):
    ...
    t, noisy_signal = scipy.signal.istft(X[i, :, :, 0]+X[i, :, :, 1]*1j, fs=config.fs,
                                   nperseg=config.nperseg, nfft=config.nfft, boundary='zeros')
    t, ideal_denoised_signal = scipy.signal.istft((X[i, :, :, 0]+X[i, :, :, 1]*1j)*Y[i, :, :, 0],
                                     fs=config.fs, nperseg=config.nperseg, nfft=config.nfft, boundary='zeros')
    t, denoised_signal = scipy.signal.istft((X[i, :, :, 0]+X[i, :, :, 1]*1j)*preds[i, :, :, 0],
                                     fs=config.fs, nperseg=config.nperseg, nfft=config.nfft, boundary='zeros')
